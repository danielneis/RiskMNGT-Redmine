...
#=RisksController
#===It manages the risks of a specific project
class ProjectRisksController < BaseRiskApplicationController
	...
	before_filter :find_project_risk ,
  ...
  #=index
  #===It shows a paginated list of project risks which can be filtered via mitigation status, category, probability or impact
  def index	  	
  ...
  #=update
  #===If the request is a post and has the project risk parameters, it updates the project risk and if it is successfully saved, redirects to index
  ...
  #=preview
  #=== It renders to "common/preview" either params[:project_risk][:mitigation] when params[:opt] == 'mitigation' or params[:project_risk][:contingency] when params[:opt] == 'contingency'
  ...
  def setSelectableRisks(risk_category_id)	
	  @select_risks = Risk.find :all,
	   	:conditions=>["status = ? AND risk_category_id=?",Risk::STATUS_ACTIVE.to_i, risk_category_id ]
  ...
  #=== It creates the query condition to be used on a project risk search
  ...
  #===It renders to _render_404 when the _project_risk_ cannot be found
