...
#=ProjectRiskStatisticsController
#===It manages the project risk statistics of a specific project
class ProjectRiskStatisticsController < BaseRiskApplicationController
  ...
  #=index
  #===It shows an specific report of project risks or incidents where the report is selected via params[:tab] {risk_impact||risk_probability||risk_status||risk_impact||risk_category||incident_status||incident_impact}
  ...
  #=risk_exposure
  #===It shows the report with the count of project risk per number of risk exposure.
  private
  def risk_exposure
  ...
  def risk_impact	  	  	
	  ...
	  data = ActiveRecord::Base.connection.select_all("select  impact as value,
                                                        count(id) as count
  ...
  def risk_probability	  	  	
	  data = ActiveRecord::Base.connection.select_all("select  probability as value,
                                                        count(id) as count
						from #{ProjectRisk.table_name} 						
                                                where project_id=#{@project.id}
  ...
  def risk_category
	  ...
	  data = ActiveRecord::Base.connection.select_all("select  risk_category_id as value,
                                                        count(id) as count
                                                from #{ProjectRisk.table_name} 						
                                                where project_id=#{@project.id}
  ...
  def incident_status
	  ...
	  data = ActiveRecord::Base.connection.select_all("select  correction_status as value,
                                                        count(id) as count
