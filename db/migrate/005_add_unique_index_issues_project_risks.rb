...
class AddUniqueIndexIssuesProjectRisks < ActiveRecord::Migration
  def self.up
	  add_index(:issues_project_risks, [:project_risk_id, :issue_id], :unique => true)
	  #add_index "issues_project_risks", :column => [:project_risk_id, :issue_id] , :unique => true
  end

  def self.down
	  remove_index(:issues_project_risks, [:project_risk_id, :issue_id])
	  #remove_index "issues_project_risks", :column => [:project_risk_id, :issue_id]	
  end
end
